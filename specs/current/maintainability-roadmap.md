...
| W3 | Partial | Migrate project-owned code to TypeScript | R1 | W2 for highest-value shared types | Daemon, root scripts, and e2e support now use TypeScript sources; daemon compiles to `apps/daemon/dist`; residual JS is checked by `pnpm guard`. Core daemon modules still rely on `// @ts-nocheck` in files such as `apps/daemon/src/server.ts`, `apps/daemon/src/db.ts`, `apps/daemon/src/agents.ts`, and `apps/daemon/src/runs.ts`, so the remaining work is to remove `@ts-nocheck` from high-risk daemon modules and type their payload, DB row, process, and SSE surfaces. |
