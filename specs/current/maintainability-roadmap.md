...
| W3 | Completed | Migrate project-owned code to TypeScript | R1 | W2 for highest-value shared types | Daemon source and tests have all `@ts-nocheck` suppressions removed and pass `pnpm --filter @open-design/daemon typecheck`; workspace `pnpm typecheck` and `pnpm guard` pass. Residual JS remains limited to the guard allowlist. |
